...
function Page() {
    const featuredCourses = coursesData.courses.filter((course) => course.isFeatured == false)
    return (
        <div className="min-h-screen bg-black py-12 pt-36">
            <h1 className="text-lg md:text-5xl text-center font-sans font-bold mb-8 text-white">
                UnFeatured Course ({featuredCourses.length})
            </h1>
                ...
                {featuredCourses.map((course) => (
                    <CardContainer className="inter-var m-4" key={course.id}>
                        <CardBody className="bg-gray-50 relative group/card  dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-auto sm:w-[30rem] h-auto rounded-xl p-6 border  ">
                            ...
                            >
                                <span className=" text-gray-100">Instructor:</span> {course.instructor}
                            </CardItem>
                            <CardItem
                                as="p"
                                translateZ="60"
